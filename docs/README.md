...
|---|---|---|
| 1 | [**Cloudflare Workers & Serverless**](1-cloudflare-workers.md) | Tìm hiểu về Điện toán đám mây thế hệ mới, chạy code tại Edge và V8 Isolates. |
| 2 | [**Hono Framework**](2-hono-framework.md) | Web framework siêu nhẹ, cực kỳ nhanh chạy trực tiếp trên môi trường Edge. |
| 3 | [**Cloudflare KV (Key-Value Store)**](3-cloudflare-kv.md) | Hệ thống lưu trữ dữ liệu NoSQL phân tán toàn cầu, cơ chế TTL và Eventual Consistency. |
| 4 | [**Security & Cryptography (Bảo mật & Mã hóa)**](4-cryptography-security.md) | Tại sao phải băm SHA-256? Web Crypto API hoạt động thế nào trong Cloudflare Workers? |
| 5 | [**Webhook & Google Apps Script (GAS)**](5-webhooks-gas.md) | Giải pháp gửi email miễn phí, an toàn mà không cần dựng SMTP Server riêng. |
| 6 | [**CORS (Cross-Origin Resource Sharing)**](6-cors-flow.md) | Tại sao trình duyệt chặn API của bạn? Cơ chế hoạt động của CORS và cách giải quyết. |
...
| 8 | [**JSON Web Token (JWT)**](8-JWT.md) | Xác thực không trạng thái (Stateless), cấu trúc token, cách ký và xác thực bằng thuật toán HS256. |
| 9 | [**Authentication & Authorization**](9-authen-author.md) | Phân biệt xác thực danh tính và ủy quyền dữ liệu, cách áp dụng middleware và bảo mật nhiều tầng. |

---

## Lời Khuyên Khi Đọc Tài Liệu

- **Đối với người mới bắt đầu**: Nên đọc theo thứ tự từ **Chuyên đề 1** đến **Chuyên đề 9** để nắm được bức tranh toàn cảnh từ hạ tầng (Workers) cho đến logic ứng dụng, bảo mật và thiết kế giao diện.
